...
$vmsize = 'Standard_A1'
$osDiskVhdUri1 = "https://$devopsEastStorage.blob.core.windows.net/vhds/"+$server+"_os.vhd"
$appDiskVhdUri1 = "https://$devopsEastStorage.blob.core.windows.net/vhds/"+$server+"_app.vhd"
...
#$utlAppDisk = Add-AzureRmVMDataDisk -VM $server -Name "$server"+"_appDisk" -VhdUri $appDiskVhdUri1 -DiskSizeInGB '80'
#endregion
#$VirtualMachine = Add-AzureRmVMDataDisk -VM $VirtualMachine -Name "disk1" -SourceImageUri $DataImageUri -VhdUri $DataDiskUri -Lun 0 -DiskSizeinGB 10 -CreateOption FromImage
...
#Add-AzureRmVMDataDisk -VM $vmconfig.Name -VhdUri $appDiskVhdUri -Name ("$server+'_appDisk'") -Caching ReadWrite -DiskSizeInGB '80' | `
...
$cmds = 'cd /home/devopsadmin/chef-starter/.chef', "knife bootstrap windows winrm $server -x \devopsadmin -P Password123$ --node-name $server --run-list 'ehbis'
#''recipe[windows]','recipe[PSModules]','recipe[W2K12R2_IIS]','recipe[testehb]','recipe[ehb-is]','recipe[common_installs]''"
