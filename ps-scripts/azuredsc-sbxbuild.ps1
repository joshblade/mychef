...
$networkRG = 'rg-vnet'
$myResourceGroup = New-AzureRmResourceGroup -Name ('rg-' + $server) -Location $location
...
$subnetIdaddressPrefix = $subnetInfo.AddressPrefix 
$subnetInfo.Id
$frontendSubnet = New-AzureRmVirtualNetworkSubnetConfig -Name ('FESubnet-' + $server) -AddressPrefix $subnetIdaddressPrefix 
#'/subscriptions/765843cc-b249-4a49-bf05-ac76e27dab94/resourceGroups/rg-vnet/providers/Microsoft.Network/virtualNetworks/vnet/subnets/subnet-devops'
$user = "devopsadmin"
...
$pip = New-AzureRmPublicIpAddress -Name ('vip-' + $server) -ResourceGroupName $myResourceGroup.ResourceGroupName -Location $location -AllocationMethod Dynamic -DomainNameLabel $server.ToLower()
$frontendIP = New-AzureRmLoadBalancerFrontendIpConfig -Name ('LB-Frontend-' + $server) -PublicIpAddress $pip
$nic = New-AzureRmNetworkInterface -Force -Name ('nic-' + $server) -ResourceGroupName $networkRG -Location $location -SubnetId $subnetID -PublicIpAddressId $pip.Id
...
Publish-AzureRmVMDscConfiguration -ConfigurationPath D:\Documents\Scripts\PowerShell\DSC\iisInstall.ps1 -ResourceGroupName $automationAccountName -StorageAccountName $automationAccountName -force
...
#endregion
$fqdn = (Get-AzureRmPublicIpAddress -ResourceGroupName $myResourceGroup).DnsSettings.Fqdn | Select-String -SimpleMatch $server
Register-AzureRmAutomationDscNode -AzureVMName $server -AutomationAccountName $automationAccountName -NodeConfigurationName iisinstall -ConfigurationMode ApplyAndAutocorrect -ConfigurationModeFrequencyMins 15 -RefreshFrequencyMins 30 -RebootNodeIfNeeded $false -ActionAfterReboot ContinueConfiguration -AllowModuleOverwrite $false -AzureVMLocation $automationLocation -ResourceGroupName $automationRG
