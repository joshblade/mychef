#Description: DSC script for building SBX servers. I am not sure if this is the final one
<# Instructions for script use
Copy the required modules from: \\hrsafs\ise\_RB\requiredDSCModules to C:\Program Files\WindowsPowerShell\Modules on the system you are going to build
This is required because we have modified some of the modules

Please copy \\hrsafs\ise\Tools\Powershell-5-Prod-Release\Win8.1AndW2K12R2-KB3066437-x64.msu to local desktop and install
...
Copy-Item -Path "\\hrsafs\ise\_RB\requiredDSCModules" -Recurse -Container "C:\Program Files\WindowsPowerShell\Modules"
...
$SACredential = (Get-Credential -UserName "sa" -Message "sa") 
   ...
   #################User Setup
       ...
       MembersToInclude = 'gemsapp','ehbssas','ehbsqlsa','reisys\ehrsa-cm', 'reisys\hrsa-cm','reisys\HRSA-DBA'
  ...
  ################WEB CONFIG#################
         ...
         WindowsFeature ASHTTPActivation
       {
        ...
        {
         Setscript ={
         $rdebug = "\\hrsafs\ISE\_BD\rtools_setup_x64.exe /quiet /norestart"
        ...
        <#
        Removed 10/22/2015 due to security issue
        Package MSXML4
        {
            Ensure = "Present"  # You can also set Ensure to "Absent"
...
############SMTP Configuration
<#
        REISys_SMTPDelivery smarthost
        {
        SmtpSvc = "SmtpSvc/1"
        Ensure = "Present"
        SmartHost = "smtpgw.reisys.com"
        }
        reisys_SMTPAccess test
        {
        Ensure = "Present"
        SmtpSvc = "SmtpSvc/1"
        IPGrant = "127.0.0.1, 255.255.255.255"
        }
        #>
        xService SMTP 
        {
...
SBXALL -ConfigurationData $test -OutputPath 'Y:\rburke\sbx'
